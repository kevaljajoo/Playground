...
class Main 
{
    ...
    public static void right_rotate_odd_elem(int arr_size, int arr[], int no_of_rotation)
    {
       ...
       if(arr_size % 2 == 1)
       {
          l_odd_elem = arr_size - 1;
       }
       else
       {
          l_odd_elem = arr_size - 2;
       }
       for(int rotation = 1; rotation <= no_of_rotation; rotation++)
       {
           int temp = arr[l_odd_elem];
           for(int index = (l_odd_elem - 2); index >= 0; index = index - 2)
            {
               arr[index + 2] = arr[index];
           }
           arr[f_odd_elem] = temp;
